...
int main() {
    ...
    for(int i = 0; i < n; i++) {
        ...
        cout << "Enter Priority (lower number = higher) for Process " << i + 1 << ": ";
    ...
    while(completed < n) {
        int idx = -1;
        int highestPrio = 1000000000;
        for(int i = 0; i < n; i++) {
            if(!done[i] && procs[i].arrivalTime <= currentTime && procs[i].priority < highestPrio) {
                highestPrio = procs[i].priority;
                idx = i;
            }
        }
