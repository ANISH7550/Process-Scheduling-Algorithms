...
int main() {
    int n;
    cout << "=== SRTF Scheduling Algorithm (Preemptive SJF) ===\n";
    ...
    while(completed < n) {
        ...
        for(int i = 0; i < n; i++) {
            if(!done[i] && procs[i].arrivalTime <= currentTime && procs[i].remainingTime < minRem && procs[i].remainingTime > 0) {
        ...
        if(procs[idx].remainingTime == 0) {
            procs[idx].turnaroundTime = currentTime - procs[idx].arrivalTime;
